 ...
 *
 */
public final class JABXParseR15 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR15(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		if (stockID != null && !stockID.equals("")) {
			...
			oldLot = item.getOldLot();
			if (oldLot == null) {// 若ZJABXWatchOldLot物件不存在，則new一個物件，設定至ZJABXQuoteItem中
		...
		try{
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_OLDLOT);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR15.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
