 ...
 *
 */
public final class JABXParseP50 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseP50(IJABXLog jabxLog) {
		...
		try{
			...
			while (nRead < dataAry.length) {
				...
				case 4:// 證券全代號
					...
					quoteDataItem.setID(fixProc.getValue(tagValue.getValue(), ""));
					break;
				case 5:// 回補訊息長度
				...
				case 6:// 回補訊息內容
					...
					if (quoteDataItem != null) {
						quoteDataItem.setData(new String(rebuildAry));
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_DOWNLOAD_QUOTEDATA);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseP50.parse()", e.getMessage());
			e.printStackTrace();
		}
	}
}