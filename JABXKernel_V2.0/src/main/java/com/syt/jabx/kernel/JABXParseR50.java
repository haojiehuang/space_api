 ...
 *
 */
public final class JABXParseR50 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR50(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXSmartShortOverview ssoObj = new ZJABXSmartShortOverview();
		try{
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, ssoObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR50.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
