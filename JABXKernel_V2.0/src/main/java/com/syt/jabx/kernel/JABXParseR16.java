 ...
 *
 */
public final class JABXParseR16 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR16(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXWatchVirtualTrade vtObj = null;
		...
		if (stockID != null && !stockID.equals("")) {
			...
			vtObj = item.getVirtualTrade();
			if (vtObj == null) {// 若ZJABXWatchVirtualTrade物件不存在，則new一個物件，設定至ZJABXQuoteItem中
				vtObj = new ZJABXWatchVirtualTrade();
			}
			item.setVirtualTrade(vtObj);
		}else {
			return;
		}
		...
		try{
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_VIRTUALTRADE);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR16.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
