 ...
 *
 */
public final class JABXParseR14 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR14(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXWatchDetailOrderOverview allDetailOrder = null;
		...
		if (stockID != null && !stockID.equals("")) {
			...
			allDetailOrder = item.getDetailOrderOverview();
			if (allDetailOrder == null) {// 若ZJABXAllDetailOrder物件不存在，則new一個物件，設定至ZJABXQuoteItem中
				allDetailOrder = new ZJABXWatchDetailOrderOverview();
				item.setDetailOrderOverview(allDetailOrder);
			}else {// 若ZJABXAllDetailOrder物件存在，則清除物件中之數據
				allDetailOrder.clear();
			}
		}else {
			return;
		}
		...
		try{
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_DETAILORDER);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR14.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
