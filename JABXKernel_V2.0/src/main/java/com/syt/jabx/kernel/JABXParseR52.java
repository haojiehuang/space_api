 ...
 *
 */
public final class JABXParseR52 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR52(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXSmartMasterOverview smoObj = new ZJABXSmartMasterOverview();
		try{
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, smoObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR52.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
