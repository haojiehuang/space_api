 ...
 *
 */
public final class JABXParseI06 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseI06(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXAdvertisementOverview aoObj = new ZJABXAdvertisementOverview();
		try {
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, aoObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseI06.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
