...
/**
 * 解析回補多筆總委買賣量筆及成交筆資訊息之類別
 * @author Jason
 *
 */
public final class JABXParseR07 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR07(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXWatchTotRefInfoOverview allTotRefInfo = null;
		...
		if (stockID != null && !stockID.equals("")) {
			...
			allTotRefInfo = item.getTotRefInfoOverview();
			if (allTotRefInfo == null) {// 若ZJABXAllTotRefInfo物件不存在，則new一個物件，設定至ZJABXQuoteItem中
				allTotRefInfo = new ZJABXWatchTotRefInfoOverview();
				item.setTotRefInfoOverview(allTotRefInfo);
			}else {// 若ZJABXAllTotRefInfo物件存在，則清除物件中之數據
				allTotRefInfo.clear();
			}
		}else {
			return;
		...
		try{
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_TOTREFINFO);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR07.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
