...
/**
 * 解析廣播訊息-FreeFormat訊息之類別
 * @author Jason
 *
 */
public final class JABXParseFreeMsgBroadcast extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseFreeMsgBroadcast(IJABXLog jabxLog) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		int fixdataLengthType = ZJABXQuoteINI.getFixdataLengthType();

		if (fixdataLengthType == JABXConst.FIX_DATA_LENGTH_NUM) {
			parse_95(fixProc, result, dataAry, ctrlHeader, offset);
		}else {
			parse_1(fixProc, result, dataAry, ctrlHeader, offset);
		}
	}
		...
		try{
			...
			while (nRead < dataAry.length) {
				...
				case 1:// 資料長度
					...
					if (dataLength != 0) {
						fixDataLength = dataLength + 3;// 讀取回補訊息之Tag(總長度length + 長度(2=加最後一分隔符號(0x01)))
					}
					break;
				default:
					fixDataLength = 0;// 訊息內容讀完，要將fixDataLength重置為0
					bulletin = fixProc.getValue(tagValue.getValue(), JABXConst.ABX_CHARSET);
					break;
				}
			}
			...
			if (offset == 0) {
				...
				result.put(JS_Result.STATUS_ID, JABXConst.ABXMSG_SYSTEMBULLETIN);
				result.put(JS_Result.DATA, bulletin);
			...
			jabxLog.outputErrorAndLog("JABXParseFreeMsgBroadcast.parse()", e.getMessage());
			e.printStackTrace();
		}		
	}

	private void parse_95(final ZJABXFixProc fixProc, final JSONObject result,
			byte[] dataAry, JABXCtrlHeader ctrlHeader, int offset) {
		String bulletin = "";
		try{
			JABXTagValue tagValue;
