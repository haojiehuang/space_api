 ...
 *
 */
public final class JABXParseWatchOthers extends IJABXParseBase implements IJABXParseBody {
	 ...
	 * @param srcObj - Object
	 */
	public JABXParseWatchOthers(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, Object srcObj) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		try{
			...
			ZJABXWatchOthers stkOthers = item.getOthers();
			if (stkOthers == null) {
				stkOthers = new ZJABXWatchOthers();
				item.setOthers(stkOthers);
			}
			StringBuffer parameterSb = new StringBuffer("000000000000000");
			...
			while (nRead < dataAry.length) {
				...
				switch (tag) {
				case 1:// 今開價
					stkOthers.setOpenPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(0, 1, "1");
					break;
				case 2:// 今高價
					stkOthers.setHighPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(1, 2, "1");
					break;
				case 3:// 今低價
					stkOthers.setLowPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(2, 3, "1");
					break;
				case 4:// 今未平倉量
					stkOthers.setTodayOI(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(3, 4, "1");
					break;
				case 5:// 結算價
					stkOthers.setSettlementPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(4, 5, "1");
					break;
				case 6:// 均買價
					stkOthers.setAvgBidPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(5, 6, "1");
					break;
				case 7:// 均賣價
					stkOthers.setAvgAskPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(6, 7, "1");
					break;
				case 8:// 合約高
					stkOthers.setContractHigh(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(7, 8, "1");
					break;
				case 9:// 合約低
					stkOthers.setContractLow(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(8, 9, "1");
					break;
				case 10:// 定價成交量
					stkOthers.setFixPriceVolume(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(9, 10, "1");
					break;
				case 11:// 衍生最佳買價(for TAIFEX)
					stkOthers.setDerivedBidPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(10, 11, "1");
					break;
				case 12:// 衍生最佳買量(for TAIFEX)
					stkOthers.setDerivedBidVolume(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(11, 12, "1");
					break;
				case 13:// 衍生最佳賣價(for TAIFEX)
					stkOthers.setDerivedAskPrice(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(12, 13, "1");
					break;
				case 14:// 衍生最佳賣量(for TAIFEX)
					stkOthers.setDerivedAskVolume(fixProc.getIntValue(tagValue.getValue()));
					parameterSb.replace(13, 14, "1");
					break;
				case 15:// 按盤價(for HK)
					stkOthers.setNormalPrice(fixProc.getIntValue(tagValue.getValue()));
			...
			if (offset == 0) {
				...
				result.put(JS_Result.STATUS_ID, JABXConst.ABXMSG_OTHERS);
			...
			jabxLog.outputErrorAndLog("JABXParseWatchOthers.parse()", e.getMessage());
			e.printStackTrace();
		}
	}
}