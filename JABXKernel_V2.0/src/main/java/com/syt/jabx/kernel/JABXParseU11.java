 ...
 *
 */
public final class JABXParseU11 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseU11(JABXKernel jabxKernel, IJABXLog jabxLog) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXUserFeedback ufObj = new ZJABXUserFeedback();
		try{
			...
			int errorCode = 0;
			String errorDesc = "";
			ZJABXUserFeedbackItem ufItem = null;
			while (nRead < dataAry.length) {
				...
				case 3:// 資料總筆數
					break;
				case 4:// 意見反饋日期時間
					ufItem = new ZJABXUserFeedbackItem();
					ufItem.setTime(fixProc.getValue(tagValue.getValue(), ""));
					break;
				case 5:// 意見反饋序號
					if (ufItem != null) {
						ufItem.setFeedbackNo(fixProc.getIntValue(tagValue.getValue()));
					}
					break;
				case 6:// 意見反饋標題
					if (ufItem != null) {
						ufItem.setTitle(fixProc.getValue(tagValue.getValue(), JABXConst.ABX_CHARSET));
						ufObj.addItem(ufItem);
					}
					break;
				}
			}
			...
			result.put(JS_Result.DATA, ufObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseU11.parse()", e.getMessage());
			e.printStackTrace();
		}
	}
}