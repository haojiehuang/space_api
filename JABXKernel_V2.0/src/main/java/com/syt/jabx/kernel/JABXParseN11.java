 ...
 *
 */
public final class JABXParseN11 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseN11(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXNewsOverview noObj = new ZJABXNewsOverview();
		try {
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, noObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseN11.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
