 ...
 *
 */
public final class JABXParseI02 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseI02(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXMarketReportOverview mpoObj = new ZJABXMarketReportOverview();
		try {
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, mpoObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseI02.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
