 ...
 *
 */
public final class JABXParseR06 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR06(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXWatchTradeOverview allTrade = null;
		...
		if (stockID != null && !stockID.equals("")) {
			...
			if (jabxKernel.isReservedData()) {
				allTrade = item.getTradeOverview();
				if (allTrade == null) {// 若ZJABXAllTrade物件不存在，則new一個物件，設定至ZJABXQuoteItem中
					allTrade = new ZJABXWatchTradeOverview();
					item.setTradeOverview(allTrade);
				}else {// 若ZJABXWatchTradeOverview物件存在，則清除物件中之數據
					allTrade.clear();
				}
			}else {
				allTrade = (ZJABXWatchTradeOverview)quoteTool.getWtoMapItem(stockID);
				if (allTrade == null) {
					allTrade = new ZJABXWatchTradeOverview();
				}else {
					allTrade.clear();
				}
				quoteTool.putWtoMapItem(stockID, allTrade);
				item.setTradeOverview(allTrade);
			}
		}else {
			return;		
		...
		try{
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_TRADE);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR06.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
