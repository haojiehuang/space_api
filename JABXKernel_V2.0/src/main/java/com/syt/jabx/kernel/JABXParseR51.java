 ...
 *
 */
public final class JABXParseR51 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR51(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXSmartRankOverview sroObj = new ZJABXSmartRankOverview();
		try{
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, sroObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR51.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
