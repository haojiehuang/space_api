 ...
 *
 */
public final class JABXParseWatchExchangeStatus extends IJABXParseBase implements IJABXParseBody {
	 ...
	 * @param srcObj - Object
	 */
	public JABXParseWatchExchangeStatus(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, Object srcObj) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		try{
			...
			JSONObject exchangeStatus = item.getExchangeStatus();
			if (exchangeStatus == null) {
				exchangeStatus = new JSONObject();
				item.setExchangeStatus(exchangeStatus);
			}
			...
			while (nRead < dataAry.length) {
				...
				switch (tag) {
				case 1:// 當地日期時間
					exchangeStatus.put(JSConst.WATCH_LOCAL_TIME, fixProc.getLongValue(tagValue.getValue()));
					break;
				case 2:// 交易狀態
					exchangeStatus.put(JSConst.WATCH_TRADE_STATUS, fixProc.getValue(tagValue.getValue(), ""));
					break;
				case 3:// 最後收盤日
					exchangeStatus.put(JSConst.WATCH_LAST_CLOSEDATE, fixProc.getIntValue(tagValue.getValue()));
					break;
				case 4:// 最後交易日
					exchangeStatus.put(JSConst.WATCH_LAST_TRADEDATE, fixProc.getIntValue(tagValue.getValue()));
					break;
				case 5:// 市場特殊狀態
					exchangeStatus.put(JSConst.WATCH_MARKET_STATUS, fixProc.getValue(tagValue.getValue(), ""));
					break;
				}
			}
			if (offset == 0) {
				result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REALTIME);
				result.put(JS_Result.STATUS_ID, JABXConst.ABXMSG_EXCHANGESTATUS);
				result.put(JS_Result.PARAM, item.getStkID());
				result.put(JS_Result.DATA, exchangeStatus);
			...
			jabxLog.outputErrorAndLog("JABXParseWatchExchangeStatus.parse()", e.getMessage());
			e.printStackTrace();
		}
	}
}