 ...
 *
 */
public final class JABXParseR96 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR96(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
		...
		try{
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_ONEMINUTETRADE);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR96.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
