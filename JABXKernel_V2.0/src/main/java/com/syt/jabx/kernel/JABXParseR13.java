 ...
 *
 */
public final class JABXParseR13 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseR13(JABXKernel jabxKernel, IJABXLog jabxLog, JABXQuoteTool quoteTool, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXWatchDetailTradeOverview allDetailTrade = null;
		...
		if (stockID != null && !stockID.equals("")) {
			...
			allDetailTrade = item.getDetailTradeOverview();
			if (allDetailTrade == null) {// 若ZJABXAllDetailTrade物件不存在，則new一個物件，設定至ZJABXQuoteItem中
				allDetailTrade = new ZJABXWatchDetailTradeOverview();
				item.setDetailTradeOverview(allDetailTrade);
			}else {// 若ZJABXAllDetailTrade物件存在，則清除物件中之數據
				allDetailTrade.clear();
			}
		}else {
			return;
		}
		...
		try{
			...
			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_REBUILD_DETAILTRADE);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseR13.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
