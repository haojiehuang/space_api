 ...
 *
 */
public final class JABXParseN02 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseN02(JABXKernel jabxKernel, IJABXLog jabxLog) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXExchangeBulletinContent ebcObj = new ZJABXExchangeBulletinContent();
		try{
			...
			while (nRead < dataAry.length) {
				...
				case 2:// 錯誤訊息
					...
					break;
				case 3:// 公告訊息日期時間
					ebcObj.setDataTime(fixProc.getValue(tagValue.getValue(), JABXConst.ABX_CHARSET));
					break;
				case 4:// 公告訊息標題
					ebcObj.setTitle(fixProc.getValue(tagValue.getValue(), JABXConst.ABX_CHARSET));
					break;
				case 5:// 相關股票代碼
					...
					ebcObj.setListOfStockID(listOfStockID);
					break;
				case 6:// 公告訊息內容長度
				...
				case 7:// 公告訊息內容
					...
					ebcObj.setData(fixProc.getValue(tagValue.getValue(), JABXConst.ABX_CHARSET));
					break;
				}
			}
			...
			String sExchangeID = paramList.get(0);
			int serialNo = Integer.parseInt(paramList.get(1));
			ebcObj.setExchangeID(sExchangeID);
			ebcObj.setSerialNo(serialNo);
			paramList = null;

			result.put(JS_Result.FUNC_ID, JABXConst.ABXFUN_QUERY_EXCHANGEBULLETINCONTENT);
			result.put(JS_Result.STATUS_ID, requestID);
			result.put(JS_Result.ERR_CODE, errorCode);
			result.put(JS_Result.ERR_DESC, errorDesc);
			result.put(JS_Result.DATA, ebcObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseN02.parse()", e.getMessage());
			e.printStackTrace();
		}
	}
}