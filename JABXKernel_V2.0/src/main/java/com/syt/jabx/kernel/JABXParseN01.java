 ...
 *
 */
public final class JABXParseN01 extends IJABXParseBase implements IJABXParseBody {
	 ...
	 */
	public JABXParseN01(JABXKernel jabxKernel, IJABXLog jabxLog, IJABXParseFactory parseFactory) {
	...
	public void parse(final ZJABXFixProc fixProc, final JSONObject result,
		...
		ZJABXExchangeBulletinOverview eboObj = new ZJABXExchangeBulletinOverview();
		try {
			...
			while(nRead < dataAry.length) {
				...
				case 5:// 回補訊息內容
					...
					parseRebuildCountData(jabxKernel, jabxLog, parseFactory, fixProc, result, rebuildAry, map, eboObj);
			...
			result.put(JS_Result.NOTIFIED, true);
			jabxLog.outputErrorAndLog("JABXParseN01.parse()", e.getMessage());
			e.printStackTrace();
		}catch(IOException e) {
